// =================================================================================
// [v18.5 최종] 백업본 원본 유지 + AND/OR 검색 및 실전/Tip 연동 완료
// =================================================================================
...
function openModal(lesson) {
    ...
    if (lesson.pdf) {
        ...
        showPdfBtn.disabled = true; showPdfBtn.innerText = '교재 없음'; showPdfBtn.dataset.pdfId = '';
    }
    ...
    if (lesson.videoid) {
        updateVideoUI(true); setupPlayer(lesson);
    } else {
...
function renderChapters(data) {
    ...
    data.forEach(chap => {
        let colorClass = 'grade-etc'; 
        const g = (chap.grade || "").toString();

        if (g.includes('실전')) colorClass = 'grade-실전';
        else if (g.includes('Tip')) colorClass = 'grade-Tip';
        else if (g.includes('중1')) colorClass = 'grade-중1';
        else if (g.includes('중2')) colorClass = 'grade-중2';
        else if (g.includes('중3')) colorClass = 'grade-중3';
        else if (g.includes('고1')) colorClass = 'grade-고1';
        else if (g.includes('고2')) colorClass = 'grade-고2';
        else if (g.includes('고3')) colorClass = 'grade-고3';

        const chapterEl = document.createElement('div');
        chapterEl.className = `chapter ${colorClass}`;
        chapterEl.innerHTML = `
            <div class="chapter-header">
                <div><h3 class="grade-unit-line"><span class="grade-label">${chap.grade}</span><span class="unit-label">${chap.unit}</span></h3>
                <h4 class="chapter-line"><span class="chapter-label">${chap.chapter}</span><span class="lesson-count">(${chap.lessons.length}강)</span></h4></div>
            </div>
        ...
        chapterEl.querySelector('.chapter-header').onclick = () => {
            const active = chapterEl.classList.contains('active');
            list.querySelectorAll('.chapter.active').forEach(c => { c.classList.remove('active'); c.querySelector('.lessons').style.maxHeight = '0px'; });
            if (!active) { chapterEl.classList.add('active'); const lEl = chapterEl.querySelector('.lessons'); lEl.style.maxHeight = lEl.scrollHeight + 'px'; }
        };
        chapterEl.querySelectorAll('.lesson a').forEach((link, idx) => {
            link.onclick = (e) => {
                ...
                if (isMobile()) {
                    if (lesson.videoid) {
                        window.open(`https://www.youtube.com/watch?v=${lesson.videoid}${lesson.time ? '&t='+lesson.time : ''}`, '_blank');
                    } else {
                        const msg = lesson.pdf ? '강의 영상은 준비 중이며,<br>현재는 교재(PDF)만 제공됩니다.' : '해당 강의는 현재 업로드 준비 중입니다.<br>잠시만 기다려주세요!';
                        showAlertModal(msg);
                    }
...
function performSearch() {
    const box = document.getElementById('searchBox'), keyword = box.value.toLowerCase().trim();
    const chapterList = document.getElementById('chapterList'), placeholder = document.getElementById('lesson-placeholder');
    ...
    document.querySelectorAll('.filter-btn').forEach(btn => { if(btn.dataset.grade === box.value.trim()) btn.classList.add('active'); });
...
document.addEventListener('DOMContentLoaded', () => {
    const searchBtn = document.getElementById('searchBtn'), searchBox = document.getElementById('searchBox');
    ...
    const alertModal = document.getElementById('customAlertModal');
    if (alertModal) { alertModal.addEventListener('click', (e) => { if (e.target === alertModal) closeAlert(); }); }
    ...
    document.getElementById('showPdfBtn').onclick = (e) => { 
        ...
        if (pc.style.width !== '0px' && pc.style.width !== '') { 
            pc.style.width = '0'; vc.style.width = '100%'; e.target.innerText = '교재 보기'; 
        } else { 
            if (frame.src.includes("about:blank")) frame.src = getDriveLink(e.target.dataset.pdfId); 
            const w = Math.max(document.getElementById('videoContent').offsetHeight * 0.707, 600); 
            pc.style.width = w + 'px'; vc.style.width = `calc(100% - ${w}px)`; e.target.innerText = '교재 닫기'; 
        } 
    };
    ...
    if (typeof fetchAllSheets === 'function') {
        fetchAllSheets().then(data => { 
            ...
            <p>위에서 공부하고 싶은 단원명이나 키워드를 검색하면 <br>강의 목록이 나타납니다.<br>검색시 띄어쓰기는 AND, 쉼표/슬래시/|는 OR 로 검색됩니다.</p>`;
        }).catch(() => { document.getElementById('lesson-placeholder').innerHTML = `<p style="color: #f87171;">데이터 로딩 실패</p>`; });
    }
});